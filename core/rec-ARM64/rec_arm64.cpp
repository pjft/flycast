	...
	bool GenReadMemoryImmediate(const shil_opcode& op)
	{
		...
		else
		{
			...
			if (size == 8)
			{
				...
				ERROR_LOG(DYNAREC, "DEBUG: Mov: arg1: %u, arg2: %lu", w0, addr);
				Mov(w0, addr);
				GenCallRuntime((void (*)())ptr);
				ERROR_LOG(DYNAREC, "DEBUG: Str: arg1: %u, arg2: %" PRIu64 ", op.rd.reg_ptr: %p", w0, sh4_context_mem_operand(op.rd.reg_ptr()), (void*)op.rd.reg_ptr());
