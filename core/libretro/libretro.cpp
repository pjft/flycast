...
void retro_set_input_state(retro_input_state_t cb)
{
   ...
   /*for (int i = 0; i < 4; i++) {
    ...
    int m0r = input_cb(i, RETRO_DEVICE_MOUSE, 0, RETRO_DEVICE_ID_MOUSE_RIGHT);

    int l0x = input_cb(i, RETRO_DEVICE_LIGHTGUN, 0, RETRO_DEVICE_ID_LIGHTGUN_SCREEN_X);
    int l0y = input_cb(i, RETRO_DEVICE_LIGHTGUN, 0, RETRO_DEVICE_ID_LIGHTGUN_SCREEN_Y);
...
{
  ERROR_LOG(COMMON, "Updating Mouse State");
...
static void UpdateInputStateNaomi(u32 port)
{
   ...
   case MDT_LightGun:
      ...
      ERROR_LOG(COMMON, "Is Lightgun");
		 ...
		 {
      ERROR_LOG(COMMON, "Not offscreen, updating");
...
{
   ERROR_LOG(COMMON, "Updating Input State");
   if (gl_ctx_resetting)
	  return;
